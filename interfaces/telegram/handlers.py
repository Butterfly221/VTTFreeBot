...
Приём голосовых сообщений и аудио-документов, вызов core.pipeline.voice_to_text().
...
# Ссылка на очередь — устанавливается из bot.py после инициализации
...
async def handle_voice(message: Message) -> None:
    ...
    logger.info(
        "Голосовое сообщение: user_id=%d, длит=%.1fс", user_id, voice.duration
    )
...
async def _process_and_respond(
    ...
    """Общий пайплайн:  транскрибация → ответ."""
    ...
    try:
        ...
        logger.warning(
            "PipelineError: user_id=%d, err=%s", user_id, type(exc).__name__
        )
        return
